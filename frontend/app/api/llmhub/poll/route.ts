...
const POLL_TOKEN = (process.env.FAL_POLL_TOKEN ?? '').trim(); // Share POLL_TOKEN for simplicity

function authorize(req: NextRequest): NextResponse | null {
  if (!POLL_TOKEN) return null;
  const rawHeader = req.headers.get('x-llmhub-poll-token') ?? req.headers.get('authorization') ?? '';
  ...
  console.warn('[llmhub-poll] unauthorized request', {
